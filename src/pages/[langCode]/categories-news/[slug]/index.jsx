...
import { GET_CATEGORIES } from '@/utils/api/api.js'
import axios from 'axios'
import dynamic from 'next/dynamic'
const CategoryNews = dynamic(() => import('../../../../components/pagesComponent/category/CategoryNews.jsx'), { ssr: false })

// This is seo api
const fetchDataFromSeo = async (id, language_id) => {
  try {
    const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/${process.env.NEXT_PUBLIC_END_POINT}/${GET_CATEGORIES}?language_id=${language_id}&slug=${id}`);
...
if (process.env.NEXT_PUBLIC_SEO === "true") {
  serverSidePropsFunction = async (context) => {
    ...
    const currentURL = process.env.NEXT_PUBLIC_WEB_URL + `${langCode}/categories-news/` + slugValue + '/';
